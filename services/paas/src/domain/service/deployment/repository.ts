...
export class DeploymentRepository {
  ...
  public async markDeploymentRunning(
    deploymentId: string,
    container: ContainerRecord,
  ): Promise<void> {
    this.store.update(deploymentId, (existingDeploymentRecord) => {
      if (!existingDeploymentRecord) {
        throw new ContextualError(
          'Failed to make deployment as running: Deployment does not exist',
          { deploymentId },
        );
      }
      return {
  ...
  public async markDeploymentFailed(
    deploymentId: string,
    failureReason: string,
  ): Promise<void> {
    this.store.update(deploymentId, (existingDeploymentRecord) => {
      if (!existingDeploymentRecord) {
        throw new ContextualError(
          'Failed to make deployment as failed: Deployment does not exist',
          { deploymentId },
        );
      }
      return {
  ...
  public async markDeploymentCleanedUp(deploymentId: string): Promise<void> {
    this.store.update(deploymentId, (existingDeploymentRecord) => {
      if (!existingDeploymentRecord) {
        throw new ContextualError(
          'Failed to make deployment as cleaned up: Deployment does not exist',
          { deploymentId },
        );
      }

...
describe('deploy task', () => {
  ...
  test('should successfully deploy and wire up deployment', async () => {
    ...
    expect(mockDeploymentRepository.markDeploymentRunning).toHaveBeenCalledWith(
      'deployment-123',
      {
        hostname: 'service-123-deployment-123',
        port: 8080,
  ...
  test('should mark deployment as failed when container fails to start', async () => {
    ...
    expect(mockDeploymentRepository.markDeploymentFailed).toHaveBeenCalledWith(
      'deployment-123',
      'Container failed to start',
    );
    expect(
      mockDeploymentRepository.markDeploymentRunning,
    ).not.toHaveBeenCalled();
